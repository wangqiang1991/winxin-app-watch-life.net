...
Page({
  ...
  onLoad: function (options) {
    ...
    args.cateType = 'all';
    Adapter.loadGoodsCategories(args, that, API, false);
    Auth.setUserMemberInfoData(that);
    this.loadVideoList();
  ...
  loadVideoList: function () {
    API.getSettings().then(res => {
      var videoList = res.settings.videoList;
  ...
  categoryBtn: function () {
    this.setData({ showCategoryPop: !this.data.showCategoryPop })
  },


  selectColumn: function (e) {

    var moveToIndex = e.currentTarget.dataset.index;
    var categoryId = e.currentTarget.dataset.categoryid;
    var categoryName = e.currentTarget.dataset.categoryname;
    this.setData({ moveToIndex: moveToIndex, moveToIndex1: -1, categoryId: categoryId, categoryName: categoryName })
  },
  selectColumn1: function (e) {

    var moveToIndex1 = e.currentTarget.dataset.index;
    var categoryId = e.currentTarget.dataset.categoryid;
    var categoryName = e.currentTarget.dataset.categoryname;
    this.setData({ moveToIndex1: moveToIndex1, moveToIndex: -1, categoryId: categoryId, categoryName: categoryName })
  },
  onClose: function () {
  ...
  coverImage: function () {
    var that = this;
    Adapter.upLoadImage(1, that.cb_imgUpload, null, null, app, API, that);
  },
  bindSalesInfo: function (e) {
    var that = this;
    that.data.goodsMessage.sales_info = e.detail.value;
  },
  bindLinkUrl: function (e) {
    var that = this;
    that.data.goodsMessage.link_url = e.detail.value;
  },
  bindGoodsPrice: function (e) {
    var that = this;
    that.data.goodsMessage.goods_price = e.detail.value;
  },
  bindGoodsSpec: function (e) {
    var that = this;
    that.data.goodsMessage.goods_spec = e.detail.value;
  },
  bindComName: function (e) {
    var that = this;
    that.data.goodsMessage.com_name = e.detail.value;
  },
  bindAeaeInfo: function (e) {
    var that = this;
    that.data.goodsMessage.area_info = e.detail.value;
  },
  bindFormnameVal: function (e) {
    var that = this;
    that.data.goodsMessage.fromname_value = e.detail.value;
  },
  // 添加商品封面
  goodsCoverImage: function() {
    var that = this;
    this.setData({
      goodsCoverLoading:true
    })
    Adapter.upLoadImage(1, that.gd_imUploas, null, null, app, API, that);
  },
  // 商品上传回调
  gd_imUploas: function (res, opt) {
    this.setData({
      goodsCoverLoading: false
    })
    ...
    if (typeof (res.data.src) != 'undefined') {
      that.data.args.goodsCover = res.data.src;
      if (res.data.id > 0) {
  ...
  publishArticle: function (e) {
    ...
    if (videoList.length > 0) {
      for (var i = 0; i < videoList.length; i++) {
        var videokey = videoList[i].videokey;
        var vid = e.detail.value[videokey];
        var video = {};
        if (vid) {
    ...
    for (var i = 0; i < that.data.contents.length; i++) {
      contents += that.data.contents[i]['decode']['text'];
    }
    that.data.args.content = contents;
    // console.log(that.data.args.content);
    var tagids = that.data.args.tagids;
    for (var i = 0; i < tagids.length; i++) {
      if (i == 0) {

        tags += tagids[i].tag_name;
    ...
    if (that.data.args.title.trim().length == 0 || that.data.args.title.trim() == '') {
    ...
    if (that.data.args.title.trim().length > 100) {
    ...
    if (that.data.categoryId.length == 0 || that.data.categoryId == '') {
    ...
    if (!that.data.goodsMessage.sales_info.trim()) {
    ...
    if (!that.data.goodsMessage.link_url.trim()) {
    ...
    if (!that.data.goodsMessage.goods_price.trim()) {
    ...
    if (!that.data.args.goodsCover.trim()) {
    ...
    if (!that.data.goodsMessage.goods_spec.trim()) {
    ...
    if (!that.data.goodsMessage.com_name.trim()) {
    ...
    if (!that.data.goodsMessage.area_info.trim()) {
    ...
    var price = that.data.args.price;
    var integral = that.data.args.integral;
    var readPay = that.data.readPay;

    var payRequired = readPay ? 1 : 0;

    if (readPay && (!integral || !price)) {

      wx.lin.showToast({
        title: '开启付费阅读,需要填写金额和积分数.',
        icon: 'error',
        duration: 2000,
        mask: true,
        placement: 'right',
    ...
    if (!that.data.contentsLen || !that.data.args.content || that.data.args.content.trim().length == 0) {
    ...
    if (that.data.contentsLen && that.data.contentsLen > 5000) {
    ...
    var data = {
      title: that.data.args.title,
      content: that.data.args.content,
      category: categoryId,
      excerpt:'摘要',// that.data.args.excerpt,
      userid: userId,
      sessionid: sessionId,
      tags: tags,
      formid: formId,
      ...
      price: price,
      integral: integral,
      sales_info :that.data.goodsMessage.sales_info,
      link_url: that.data.goodsMessage.link_url,
      goods_price: that.data.goodsMessage.goods_price,
      goods_image: that.data.args.goodsCover,
      goods_spec: that.data.goodsMessage.goods_spec,
      com_name: that.data.goodsMessage.com_name, 
      area_info: that.data.goodsMessage.area_info,
      fromname_value: that.data.goodsMessage.fromname_value,
    };
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          ...
          API.pubGoodslishPost(data).then(res => {
            if (res.success) {
              wx.showToast({
                title: res.message,
                icon: "none",
  ...
  bindExcerpt: function (e) {
    var that = this;
    that.data.args.excerpt = e.detail.value
  },
  bindintegral: function (e) {
    var that = this;
    that.data.args.integral = e.detail.value

  },
  bindprice: function (e) {
    var that = this;
    that.data.args.price = e.detail.value
  ...
  _bindTextinput: function (e) {
    ...
    if (typeof (that.data.contents[index]) != undefined) {
      ...
      if (that.data.contents[index]['type'] == "text") {
        that.data.contents[index]['decode']['text'] = "<p>" + e.detail.detail.value + "</p>"
      }
      else {
        that.data.contents[index]['decode']['text'] = "<img src='" + that.data.contents[index]['decode']['src'] + "' />" + "<p>" + e.detail.detail.value + "</p>";
    ...
    var contentsLen = 0;
    for (var i = 0; i < that.data.contents.length; i++) {
      if (that.data.contents[i]['type'] == "text") {
  ...
  cb_imgUploadNode: function (res, opt, isEdit) {
    ...
    if (typeof (res.data.src) != 'undefined') {
      ...
      if (res.data.id > 0) {
        ...
        //   console.log('网络');
        //  console.log(res);
        if (typeof (res.data.nodeIndex) != 'undefined') {
          var nodeIndex = res.data.nodeIndex;
          //     console.log('网络');
    ...
    if (that.data.contents[nowindexs].decode.id > 0) {
      ...
      that.data.contents[nowindexs].decode.islode = res.data.islode;
    }
    that.setData({
      contents: that.data.contents
    }
  ...
  _addImage: function (e) {
    ...
    Adapter.upLoadImage(1, that.cb_imgUploadNode, nodeIndex, null, app, API, that);
  },
  // 1.3 改变排序
  ...
  _bindTagChange: function (e) {
    ...
    if (types) {
      ...
      wx.showToast({
        title: e.detail.currentTarget.dataset.name + '标签已存在',
  ...
  _onchangeimg: function (e) {
    ...
    Adapter.upLoadImage(1, that.cb_imgUploadchange, e.target.id, null, app, API, that);
  },
